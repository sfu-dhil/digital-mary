...
class CivilizationController extends AbstractController implements PaginatorAwareInterface
{
     ...
     * @param CivilizationRepository $civilizationRepository
     *
     * @Template()
     *
     * @return array
    ...
    public function search(Request $request, CivilizationRepository $civilizationRepository) {
        ...
        if ($q) {
            ...
            $civilizations = $this->paginator->paginate($query, $request->query->getInt('page', 1), $this->getParameter('page_size'), array('wrap-queries'=>true));
        } else {
            $civilizations = [];
        }
